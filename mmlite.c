...
_PG_output_plugin_init(OutputPluginCallbacks *cb)
{
	...
	cb->startup_cb = mmlite_startup;
	cb->begin_cb = mmlite_begin_txn;
	cb->change_cb = mmlite_change;
	cb->truncate_cb = mmlite_truncate;
	cb->commit_cb = mmlite_commit_txn;
	cb->filter_by_origin_cb = mmlite_origin_filter;
	cb->shutdown_cb = mmlite_shutdown;
}
						...
						List **publication_names)
{
	...
	foreach(lc, options)
	{
		...
		else if (strcmp(defel->defname, "publication_names") == 0)
		{
			...
			elog(ERROR, "unrecognized mmlite option: %s", defel->defname);
	}
}
 ...
 */
static void
mmlite_begin_txn(LogicalDecodingContext *ctx, ReorderBufferTXN *txn)
{
...
/*
 * Currently we never forward (as long as origin_id is valid).
 */
static bool
					   ...
					   RepOriginId origin_id)
{
	...
	{
		ereport(DEBUG1, (errmsg_internal("MMLITE: found valid origin, skip publishing")));
		return true;
	}
 ...
 */
static void
mmlite_shutdown(LogicalDecodingContext *ctx)
{
